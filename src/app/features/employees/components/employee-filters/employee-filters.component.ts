// features/employees/components/employee-filters/employee-filters.component.ts
          ...
          <div class="filter-buttons">
            @for (filter of roleFilters; track filter.value) {
              <button
                ...
                (click)="onRoleFilterChange(filter.value)">
                {{ filter.label }}
                @if (filter.value !== 'all') {
 ...
 `,
  styles: `
    ...
    * {
    ...
    /* Colores específicos para badges de rol cuando están activos */
    .filter-btn.active[data-role="management"] {
      background: linear-gradient(135deg, #C5A048, #8E6F2E);
    }
...
export class EmployeeFiltersComponent {
  ...
  // Contadores de roles
  receptionCount = input<number>(0);
  cleaningCount = input<number>(0);
  maintenanceCount = input<number>(0);
  securityCount = input<number>(0);
  managementCount = input<number>(0);

  // Contadores de estados
  activeCount = input<number>(0);
  inactiveCount = input<number>(0);
  onVacationCount = input<number>(0);
  sickLeaveCount = input<number>(0);
  ...
  onRoleFilterChange(role: EmployeeRole | 'all') {
    this.selectedRole = role;
    this.onRoleFilter.emit(role);
  }

  onStatusFilterChange(status: EmployeeStatus | 'all') {
