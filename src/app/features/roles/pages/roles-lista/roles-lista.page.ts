...
import { EstadoActivo, RolFilterRequest, RolResponse } from '../../models/rol.model';
import { RolFormModalComponent } from '../../components/rol-form-modal.component';
...
@Component({
  ...
  imports: [RouterLink, RolFormModalComponent],
  template: `
    <div class="space-y-6">

      <div class="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 class="text-2xl font-bold text-[#2D2926]">Roles</h1>
          <p class="text-sm text-[#2D2926]/55 mt-0.5">Roles del sistema y sus permisos</p>
        </div>
        @if (puedeCrear()) {
      ...
      <div class="bg-white rounded-2xl border border-[#EEE3D1] overflow-hidden">
        @if (loading()) {
          ...
          <div class="py-16 text-center">
            <p class="text-sm font-semibold text-[#2D2926]">Sin roles</p>
          </div>
        } @else {
          <div class="overflow-x-auto">
            <table class="w-full min-w-[700px] text-sm">
              ...
              <tbody>
                @for (r of filas(); track r.rolId) {
                  <tr class="border-b border-[#EEE3D1] last:border-0 hover:bg-[#F9F5F0] transition-colors">
                    <td class="px-4 py-3 font-medium text-[#2D2926] whitespace-nowrap">{{ r.nombre }}</td>
                    <td class="px-4 py-3 text-[#2D2926]/70">{{ r.descripcion ?? '—' }}</td>
                    <td class="px-4 py-3 text-[#2D2926]/70">{{ r.permisos.length }}</td>
                    <td class="px-4 py-3">
                    ...
                    <td class="px-4 py-3 whitespace-nowrap text-right">
                      <div class="flex items-center justify-end gap-2">
                        <a [routerLink]="['/roles', r.rolId]"
                          class="h-7 px-2.5 inline-flex items-center rounded-lg border border-[#C5A048]
                                 text-[#C5A048] text-xs font-medium hover:bg-[#C5A048]/5 transition-colors">
                          Permisos
                        </a>
                        @if (puedeEditar()) {
                          <button type="button" (click)="abrirEditar(r)"
                        ...
                        @if (puedeEliminar()) {
                          <button type="button" (click)="eliminar(r)"
          ...
          <div class="flex items-center justify-between gap-3 px-4 py-3 border-t border-[#EEE3D1]">
            <p class="text-xs text-[#2D2926]/50">
              {{ totalElements() }} rol(es) · Página {{ pageIndex() + 1 }} de {{ totalPages() }}
            </p>
...
export class RolesListaPage {
  ...
  readonly page = signal<PageResponse<RolResponse> | null>(null);
  ...
  readonly rolSel = signal<RolResponse | null>(null);
  ...
  readonly puedeCrear = computed(() => this.store.hasPermission('rol:create'));
  readonly puedeEditar = computed(() => this.store.hasPermission('rol:update'));
  readonly puedeEliminar = computed(() => this.store.hasPermission('rol:delete'));
  ...
  private cargar(): void {
    ...
    const filtros: RolFilterRequest = {
    ...
    this.svc.listar(filtros).subscribe({
      ...
      error: (err: HttpErrorResponse & { friendlyMessage?: string }) => {
        ...
        this.toastr.error(err.friendlyMessage ?? 'No se pudieron cargar los roles.', 'Error');
      },
    });
  }
  ...
  abrirNuevo(): void { this.rolSel.set(null); this.formAbierto.set(true); }
  abrirEditar(r: RolResponse): void { this.rolSel.set(r); this.formAbierto.set(true); }
  ...
  async eliminar(r: RolResponse): Promise<void> {
    const ok = await this.confirm.ask({
      title: 'Eliminar rol',
      message: `¿Eliminar el rol "${r.nombre}"? Se quitarán también sus permisos. Esta acción no se puede deshacer.`,
      confirmText: 'Sí, eliminar',
      cancelText: 'Cancelar',
      variant: 'danger',
    });
    if (!ok) return;
    this.svc.eliminar(r.rolId).subscribe({
      next: () => { this.toastr.success('Rol eliminado.'); this.cargar(); },
      error: (err: HttpErrorResponse & { friendlyMessage?: string }) => {
        this.toastr.error(err.friendlyMessage ?? 'No se pudo eliminar el rol.', 'Error');
      },
    });
  }
